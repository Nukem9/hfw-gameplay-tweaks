...
#if 0
		const static auto EntityResourceRTTI = HRZ2::RTTI::FindByName("EntityResource")->AsCompound();

		//if (rtti == HRZ2::RTTI::FindByName("HumanoidResource") && object->m_UUID == HRZ2::GGUUID::Parse("7BBFF8B0-5093-6339-AFD5-57A034199F49"))
		if (false && rtti->IsKindOf(EntityResourceRTTI))
		{
			auto recurseCompoentSet = [&](this auto&& Self, HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>& Array) -> void
			{
				for (auto& c : Array)
				{
					spdlog::info("Component: {}", c->GetRTTI()->GetSymbolName());

					if (c->GetRTTI() == HRZ2::RTTI::FindByName("CharacterDescriptionComponentResource"))
					{
						spdlog::info("ENTITYRESOURCE {} LOCALIZED UUID {}", object->m_UUID, c->m_UUID);
					}

					if (c->GetRTTI() == HRZ2::RTTI::FindByName("EntityComponentSetResource"))
						Self(c->GetMemberRefUnsafe<HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>>("ComponentResources"));
				}
			};

			auto& components = object->GetMemberRefUnsafe<HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>>("EntityComponentResources");
			recurseCompoentSet(components);
		}

		const static auto SpawnSetupConcreteRTTI = HRZ2::RTTI::FindByName("SpawnSetupConcrete")->AsCompound();

		if (rtti->IsKindOf(SpawnSetupConcreteRTTI))
		{
			auto& entityResource = object->GetMemberRefUnsafe<HRZ2::Ref<HRZ2::RTTIRefObject>>("EntityResource");
			...
			auto recurseCompoentSet = [&](this auto&& Self, HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>& Array) -> void
			{
				for (auto& c : Array)
				{
					if (c->GetRTTI() == HRZ2::RTTI::FindByName("CharacterDescriptionComponentResource"))
					{
						auto& name = c->GetMemberRefUnsafe<HRZ2::Ref<HRZ2::RTTIRefObject>>("LocalizedName");
						const auto func = Offsets::Relative(0x06B95D0).ToPointer<void(void *, char **)>();

						if (name.GetPtr())
						{
							char *temp = nullptr;
							func(name.GetPtr(), &temp);

							if (temp)
								spdlog::info("SpawnSetupName {{\"SetupUUID\": \"{}\", \"Name\": \"{}\"}}", object->m_UUID, temp);
						}
					}
			...
			auto& variantRef = object->GetMemberRefUnsafe<HRZ2::Ref<HRZ2::RTTIRefObject>>("BodyVariant");

			if (variantRef)
			{
				spdlog::info("SpawnSetupVariant {{\"SetupUUID\": \"{}\", \"VariantUUID\": \"{}\"}}", object->m_UUID, variantRef->m_UUID);
			}

			auto& components = entityResource->GetMemberRefUnsafe<HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>>("EntityComponentResources");
			recurseCompoentSet(components);

			auto& components2 = object->GetMemberRefUnsafe<HRZ2::Array<HRZ2::Ref<HRZ2::RTTIRefObject>>>("ComponentResources");
			recurseCompoentSet(components2);
		}

		if (rtti == HRZ2::RTTI::FindByName("GraphProgramResource") || rtti == HRZ2::RTTI::FindByName("ProgramResource"))
		{
			auto& eps = object->GetMemberRefUnsafe<HRZ2::Array<ProgramResourceEntryPoint>>("EntryPoints");
			spdlog::debug("{}: {}", object->m_UUID, object->GetRTTI()->GetSymbolName());

			for (auto& entry : eps)
			{
				//auto f = Offsets::Relative(0x00EF790).ToPointer<void(uint64_t *, const void *, size_t)>();

				uint64_t data[2] = {};
				//f(data, entry.m_EntryPoint.c_str(), entry.m_EntryPoint.length());

				spdlog::info("EntryPoint: {} -- {:X}", entry.m_EntryPoint, data[0]);
			}
		}

		const static auto SpawnSetupConcreteRTTI = HRZ2::RTTI::FindByName("CameraModeResource")->AsCompound();

		if (rtti->IsKindOf(SpawnSetupConcreteRTTI))
		{
			auto test = object->GetRTTI()->AsCompound()->SerializeObject(object, 8);
			spdlog::info("0x{:X}\n", reinterpret_cast<uintptr_t>(object.GetPtr()));
			spdlog::info("{}\n", test.value());
		}

		if (object->m_UUID == HRZ2::GGUUID::Parse("A1C1A4DB-B396-4743-885C-BBAE90BBB8F2") ||
			object->m_UUID == HRZ2::GGUUID::Parse("78562C5B-EE32-C548-A3B9-D1AC06009A5B") ||
			object->m_UUID == HRZ2::GGUUID::Parse("A7493033-3D69-C94E-9791-641089B1FE46"))
		{
			auto test = object->GetRTTI()->AsCompound()->SerializeObject(object, 3);
			spdlog::info("0x{:X}\n", reinterpret_cast<uintptr_t>(object.GetPtr()));
			spdlog::info("{}\n", test.value());
		}
#endif
